...
The 4-stage loop: Trigger Moment → Share Action → Referred User Converts → Reward Delivered → Loop. If any stage is weak, the loop breaks. Best triggers: post-aha moment, post-milestone, post-great-support, post-renewal. Worst: day 1, onboarding emails.

> See [references/loop-and-incentives.md](references/loop-and-incentives.md) for the full loop breakdown, share friction removal, incentive design (single vs. double-sided), reward types table, sizing rules, and tiered gamification.
...
See [references/program-mechanics.md](references/program-mechanics.md) for the full affiliate launch checklist, commission structures, partner toolkit requirements, and recruitment playbook.
