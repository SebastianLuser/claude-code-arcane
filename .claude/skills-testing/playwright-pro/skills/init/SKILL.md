...
If `.github/workflows/` exists, create `playwright.yml` with: checkout, setup-node (lts/*), `npm ci`, `npx playwright install --with-deps`, `npx playwright test`, upload playwright-report artifact (retention 30 days). Trigger on push/PR to main and dev. If `.gitlab-ci.yml` exists, add a Playwright stage instead.
