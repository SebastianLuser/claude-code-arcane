...
description: "Build.cs, Target.cs, module creation, plugin setup, build configuration"
category: "gamedev"
argument-hint: "[module o plugin]"
user-invocable: true
...
Ask before writing: this skill edits `Build.cs`, `Target.cs` and plugin descriptor files. Propose the change and get approval before using Write or Edit.

## Before Starting

Read `docs/unreal/project-context.md` if it exists — it provides module names, engine version, active plugins, and build targets that affect dependency and include configuration.
...
- [ ] Dependencies are private unless the public headers genuinely need them
- [ ] Module type and loading phase are correct for what the module does
- [ ] The project still builds for every target platform it claims to support
...
- `/ue-packaging` — produce and verify a packaged build
