...
description: "Enhanced Input system — Input Actions, Input Mapping Contexts, modifiers, triggers"
category: "gamedev"
argument-hint: "[action o input scheme]"
user-invocable: true
...
Ask before writing: this skill edits Input Action and Mapping Context assets plus binding code. Propose the change and get approval before using Write or Edit.
...
- `references/input-action-reference.md` — full trigger and modifier catalogue with parameters
- `references/enhanced-input-workflow.md` — editor-side authoring flow for actions and mapping contexts
- `references/cpp-setup.md` — C++ setup: subsystem access, context priorities, binding callbacks
...
- [ ] Mapping contexts are added and removed with explicit priorities per game state
- [ ] Input Action value types match how the action is consumed
- [ ] No legacy `BindAction`/`BindAxis` or direct key polling remains
...
- `/ue-ui-umg-slate` — verify input mode switching between game and UI
