...
description: "GameMode, GameState, PlayerController, PlayerState, Pawn, HUD"
category: "gamedev"
argument-hint: "[class o flow]"
user-invocable: true
...
Ask before writing: this skill edits GameMode, GameState, Controller, PlayerState and Pawn classes. Propose the change and get approval before using Write or Edit.

## Context Check

Read `docs/unreal/project-context.md` before proceeding. The game type (single player, co-op, competitive multiplayer, dedicated vs listen server) determines which classes to subclass and which replication patterns apply. Resolve: single-player or multiplayer? Dedicated or listen server? What are you implementing?
...
- `references/framework-class-map.md` — which framework class owns which responsibility, and what exists on server vs client
...
- [ ] State lives in the correct class — GameState for shared, PlayerState for per-player
- [ ] Nothing authoritative lives on the Pawn, which can be destroyed and respawned
- [ ] Server and client paths were both considered for every piece of state
...
- `/ue-networking-replication` — replicate the framework state
