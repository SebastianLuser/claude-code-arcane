...
description: "Niagara particle systems, VFX, emitters, data interfaces, GPU simulation"
category: "gamedev"
argument-hint: "[effect o system]"
user-invocable: true
...
Ask before writing: this skill edits Niagara system and emitter assets. Propose the change and get approval before using Write or Edit.
...
- `references/niagara-parameter-types.md` — C++ type to Niagara type mapping
- `references/niagara-data-interfaces.md` — built-in data interface catalogue
- `references/niagara-workflow.md` — editor-side authoring flow: systems, emitters, modules, parameters
...
- [ ] Particle counts and spawn rates are bounded, with scalability settings set
- [ ] GPU simulation is used where the effect is heavy and does not need CPU readback
- [ ] Effects are pooled or auto-destroyed rather than leaking components
...
- `/perf-profile` — confirm the VFX budget holds under load
