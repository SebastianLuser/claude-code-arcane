> Adapted from the `unreal-blueprints` skill in [awesome-gamedev-agent-skills](https://github.com/gamedev-skills/awesome-gamedev-agent-skills) (Apache-2.0). Cross-skill references were remapped to Arcane slash commands.

# Blueprint communication: Cast vs Interface vs Event Dispatcher (UE 5.8)

Depth for `/ue-blueprints`. The three ways Blueprints talk to each other, when to use each,
