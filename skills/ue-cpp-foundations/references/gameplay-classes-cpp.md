# Gameplay Classes in C++ — Editor-Side Workflow

> Adapted from the `unreal-cpp-gameplay` skill in [awesome-gamedev-agent-skills](https://github.com/gamedev-skills/awesome-gamedev-agent-skills) (Apache-2.0). Cross-skill references were remapped to Arcane slash commands.
...
**When *not* to use:** designer-facing visual logic → `/ue-blueprints`. Player input
binding details → `/ue-input-system`. AI logic → `/ue-ai-navigation`. This skill owns
...
- `/ue-blueprints` — exposing C++ to designers; BP/C++ interop.
- `/ue-input-system` — binding input in a C++ Pawn/Character.
- `/ue-ai-navigation` — C++ AI tasks driven from a behaviour tree.
