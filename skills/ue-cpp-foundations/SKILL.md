...
description: "UPROPERTY, UFUNCTION, UCLASS, TArray, TMap, delegates, FString, garbage collection, smart pointers"
category: "gamedev"
argument-hint: "[class, type o question]"
user-invocable: true
...
Ask before writing: this skill edits C++ headers and source files under `Source/`. Propose the change and get approval before using Write or Edit.

## Context

Read `docs/unreal/project-context.md` for engine version, coding conventions, and project-specific rules. Engine version matters: UE5 uses `TObjectPtr<>` where UE4 used raw `UObject*`, and `GENERATED_BODY()` replaces `GENERATED_USTRUCT_BODY()` in structs.
...
- `/ue-blueprints` — the visual-scripting side of the C++/Blueprint boundary

## Reference Files

- `references/property-specifiers.md` — full `UPROPERTY`/`UFUNCTION` specifier list
- `references/container-patterns.md` — `TArray`/`TMap`/`TSet` API and performance guide
- `references/delegate-patterns.md` — delegate declaration macros and binding methods
- `references/gameplay-classes-cpp.md` — editor-side workflow for the gameplay framework classes in C++
- `references/components-and-gc.md` — component ownership and garbage-collection rules
...
- [ ] Every UObject reference is a `TObjectPtr<>` with `UPROPERTY()` markup
- [ ] Containers are `TArray`/`TMap`/`TSet`, strings use the right `FName`/`FText`/`FString` role
- [ ] No `new`/`delete` on UObjects — `NewObject<>`/`CreateDefaultSubobject<>` only
...
**PASS** — every item holds. **CONCERNS** — the work stands but at least one item is unresolved and worth flagging to the user. **FAIL** — an item is violated in a way that breaks correctness, replication or the frame budget; fix it before handing the work over.

## Next Steps

- `/ue-actor-component-architecture` — apply these types to actor design
