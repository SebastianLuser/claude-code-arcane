...
description: "UAudioComponent, SoundCue, MetaSound, attenuation, concurrency, audio analysis"
category: "gamedev"
argument-hint: "[sound o audio system]"
user-invocable: true
...
Ask before writing: this skill edits audio components, MetaSound and Sound Cue assets. Propose the change and get approval before using Write or Edit.

## Context Check

Before implementing audio, read `docs/unreal/project-context.md` for:
...
- `references/audio-setup-patterns.md` — attenuation, concurrency and submix setup patterns
...
- [ ] Concurrency and priority are configured so voice count stays bounded
- [ ] Attenuation settings are shared through assets rather than tuned per instance
- [ ] Sounds are not started every frame or on every overlap event
...
- `/audio-mix` — place the new sounds in the bus hierarchy (install the `audio` profile: `arcane add +audio`)
- `/audio-audit` — check voice count, memory and loudness (same profile)
