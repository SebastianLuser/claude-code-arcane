...
description: "World Partition, level streaming, level travel, data layers, world subsystems"
category: "gamedev"
argument-hint: "[level o streaming strategy]"
user-invocable: true
...
Ask before writing: this skill edits world settings, data layers and streaming code. Propose the change and get approval before using Write or Edit.

---

## Context

Read `docs/unreal/project-context.md` before advising. Pay attention to:
...
- `references/streaming-patterns.md` — level streaming configurations: distance, data layers, streaming sources
...
- [ ] Streaming sources and distances are tuned; nothing loads the whole world
- [ ] Data layers are used for state variation instead of duplicated levels
- [ ] Level transitions handle actors that must survive travel
...
- `/perf-profile` — measure hitches at streaming boundaries
