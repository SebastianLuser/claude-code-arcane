...
description: "AI controllers, behavior trees, blackboards, AI perception, NavMesh, EQS"
category: "gamedev"
argument-hint: "[behavior o AI feature]"
user-invocable: true
...
Ask before writing: this skill edits AI controllers, behavior trees, blackboard and EQS assets. Propose the change and get approval before using Write or Edit.

## Context

Read `docs/unreal/project-context.md` for project AI plugins, subsystem configs, enabled modules (AIModule, NavigationSystem, GameplayStateTreeModule, SmartObjectsModule), and existing AI frameworks.
...
- `references/behavior-tree-workflow.md` — editor-side authoring flow: composites, decorators, services, AIController wiring
- `references/custom-bttask.md` — writing a custom `BTTask` node, including the latent-task pattern
...
- [ ] Behavior tree decorators observe blackboard keys instead of polling in tasks
- [ ] Perception configuration matches the intended detection ranges and teams
- [ ] NavMesh generation settings cover the agent sizes actually used
...
- `/ue-state-trees` — consider StateTree if the tree is getting unwieldy
