...
description: "UMG, Slate, UserWidget, HUD, BindWidget, Common UI, MVVM"
category: "gamedev"
argument-hint: "[widget o UI flow]"
user-invocable: true
...
Ask before writing: this skill edits widget classes and UMG assets. Propose the change and get approval before using Write or Edit.

## Context Check

Read `docs/unreal/project-context.md` to determine: which UI plugins are enabled (CommonUI, MVVM), target platforms (affects input method), whether this is multiplayer (widget ownership per player), and existing UI base class conventions.
...
- `references/widget-types.md` — UMG widget type catalogue with the panel/layout decision guide
...
- [ ] Widgets are data-bound or event-driven, not polling gameplay state every tick
- [ ] Focus, navigation and gamepad input were verified, not just mouse
- [ ] Widgets are pooled or properly removed rather than accumulating
...
- `/accessibility` — check contrast, scaling and navigation
- `/perf-profile` — check widget tick and paint cost
