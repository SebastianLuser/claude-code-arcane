...
description: "Sequencer, LevelSequence, cutscenes, cinematics, camera tracks, Movie Render Queue"
category: "gamedev"
argument-hint: "[sequence o shot]"
user-invocable: true
...
Ask before writing: this skill edits LevelSequence assets and sequencer-driving code. Propose the change and get approval before using Write or Edit.

## Context Check

Read `docs/unreal/project-context.md` before writing any cinematic code for:
...
- `references/sequencer-patterns.md` — common Sequencer setups: bindings, camera cuts, gameplay handoff
...
- [ ] Gameplay state is restored correctly when the sequence ends or is skipped
- [ ] Sequences are bound through spawnables or resolvable bindings, not fragile references
- [ ] Render settings are separated from in-game playback settings
...
- `/ue-world-level-streaming` — ensure required levels are loaded for the shot
